...
public class SysRoleMenuPermServiceImpl implements SysRoleMenuPermService {
    ...
    public ResponseResult findByRid(String role_id) {
        ...
        if (sysRolePerm != null) {
            ...
            sysPermPidList = sysPermList.stream().filter(sysMenu -> sysMenu.getType() == SysMenuConfig.BTN_FLAG).map(sysMenu -> sysMenu.getParent_id()).collect(Collectors.toList());
        }
    ...
    public ResponseResult updateByRid(String role_id, String menu_perm_id) {
        ...
        SysRolePerm sysRolePerm = sysRolePermMapper.findByRid(role_id);
        int updateRolePermResult = 0;
        if (sysRolePerm == null){
            sysRolePerm = new SysRolePerm();
            sysRolePerm.setRole_id(role_id);
            sysRolePerm.setPerm_id(permIds);
        ...
        SysRoleMenu sysRoleMenu = sysRoleMenuMapper.findByRid(role_id);
        int updateRoleMenuResult = 0;
        if (sysRoleMenu == null) {
            sysRoleMenu = new SysRoleMenu();
            sysRoleMenu.setRole_id(role_id);
            sysRoleMenu.setMenu_id(menuIds);
