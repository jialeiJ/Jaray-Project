...
export default {
    ...
    methods: {
        ...
        addComplaint: function(){
            ...
            // 调用接口
            API.addOrUpdateComplaint(params).then(function (result) {
                if (result.code === 200) {
                    that.initTable()
                    that.$message({
                        message: '恭喜你，添加成功',
        ...
        viewComplaint: function(row){
            ...
            API.viewComplaint(params).then(function (result) {
                if (result.code === 200) {
                    console.log(result)
                    that.form = result.map.complaintEntity
        ...
        delComplaints: function(){
            ...
            // 调用接口
            API.delComplaint(params).then(function (result) {
                if (result.code === 200) {
                    that.initTable()
                    that.$message({
                        message: '恭喜你，删除成功',
                    ...
                    for(var j=0;j<tableData.length;j++){
                        for(var item in tableData[j]){ 
                            if(item == that.tableTitle[i].prop+''){  //item 表示Json串中的属性，如'title' 
                                ...
                                console.log(that.tableTitle[i].prop, tableData[j][that.tableTitle[i].prop])
                            } 
                        }
                    }
